...
import { replacePageContentFragment } from '../../tools/page-writer'
...
import { resolveLayoutSkillName } from '../../skills/skill-contract'
...
async function resolvePageBeautifyContext(
  ctx: IpcContext,
  args: { sessionId: string; selectedPageId: string; modelConfigId?: string; layoutAudit?: string }
...
export class PageBeautifyJobService {
  ...
  async start(
    ...
    const reservation = this.coordinator.reserve('page-beautify:start', sessionId)
    ...
    try {
      ...
      if (context && jobCreated)
        await this.settleFailure(context, error, lease.controller.signal.aborted)
      if (context && !jobCreated) {
  ...
  async cancel(sessionId: string): Promise<boolean> {
    ...
    // Post-restart path: an active job exists in DB but no in-memory run is attached.
    // The lease is the only handle we have; aborting its controller alone leaves DB
    // inconsistent, so finalize the job as aborted and clean any tmp file.
    const lease = this.coordinator.get(sessionId)
    if (!lease || !this.coordinator.isOwnedBy(sessionId, 'page-beautify')) return false
    ...
    try {
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        const fragment = await runPageBeautifyAgent({
          ...
          signal: job.lease.controller.signal,
        ...
        try {
          ...
          if (job.lease.controller.signal.aborted || attempt === 1) throw error
      ...
      const cancelled =
        job.lease.controller.signal.aborted ||
      ...
      await this.settleFailure(job.context, error, job.lease.controller.signal.aborted)
    } finally {
