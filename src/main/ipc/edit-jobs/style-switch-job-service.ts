...
import { SessionJobCoordinator } from './session-job-coordinator'
...
export class StyleSwitchJobService {
  ...
  async start(
    ...
    const reservation = this.coordinator.reserve('style-switch:start', sessionId)
    if (reservation.alreadyRunning) {
      return { success: true, styleId, runId: reservation.runId, alreadyRunning: true }
    }
    ...
    try {
      ...
      context = await resolveEditContext(this.ctx, event, {
        sessionId,
        modelConfigId,
      ...
      if (jobCreated && context) {
        ...
        await this.ctx.db.updateSessionJobStatus(context.runId, 'aborted', {
          abortReason: lease.controller.signal.aborted ? 'cancelled' : 'setup_failed'
        })
        await this.ctx.db.updateGenerationRunStatus(context.runId, 'failed', message)
        this.ctx.emitGenerateChunk(sessionId, {
          type: 'run_error',
          payload: { runId: context.runId, message, cancelled: lease.controller.signal.aborted }
        })
      }
  ...
  async cancel(sessionId: string): Promise<boolean> {
    ...
    if (!lease || !this.coordinator.isOwnedBy(sessionId, 'style-switch')) return false
  ...
  private async run(job: ActiveStyleSwitchJob): Promise<void> {
    try {
      ...
      if (job.lease.controller.signal.aborted || job.fatalError) {
        ...
        await this.ctx.db.updateSessionJobStatus(job.runId, 'aborted', {
          abortReason: job.lease.controller.signal.aborted ? 'cancelled' : message
        })
        ...
        this.ctx.emitGenerateChunk(job.sessionId, {
          ...
          payload: {
            runId: job.runId,
            message,
            cancelled: job.lease.controller.signal.aborted,
  ...
  private async runWorkers(job: ActiveStyleSwitchJob): Promise<void> {
    ...
    const worker = async (): Promise<void> => {
      while (!job.lease.controller.signal.aborted && !job.fatalError) {
  ...
  private async runPage(job: ActiveStyleSwitchJob, page: StyleSwitchPageRef): Promise<void> {
    ...
    const indexPath = path.join(job.context.entry.projectDir, 'index.html')
    ...
    try {
      ...
      if (
        ...
        this.ctx.agentManager.cancelSession(job.sessionId)
      }
    }
  }
  ...
  private async commitPage(
    ...
    const relativePath = toRelativeProjectPath(job.context.entry.projectDir, page.htmlPath)
