...
export class DeckEditJobService {
  ...
  async start(event: Electron.IpcMainInvokeEvent, payload: unknown): Promise<{
    ...
    const reservation = this.coordinator.reserve('deck-edit:start', input.sessionId)
  ...
  async cancel(sessionId: string): Promise<boolean> {
    ...
    if (!job) {
      ...
      if (!lease || !this.coordinator.isOwnedBy(sessionId, 'deck-edit')) return false
  ...
  private async run(job: ActiveDeckEditJob): Promise<void> {
    ...
    try {
      await executeDeckAllPageEditGeneration(this.ctx, emitAssistant, job.context)
